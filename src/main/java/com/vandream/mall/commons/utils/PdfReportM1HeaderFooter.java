...
public class PdfReportM1HeaderFooter extends PdfPageEventHelper {
    ...
    public void onEndPage(PdfWriter writer, Document document) {
        try {
            ...
            if (fontDetail == null) {
                fontDetail = new Font(bf, presentFontSize, Font.NORMAL,new BaseColor(21,21,21));
            // 数据体字体
               }
        ...
        // 2.写入前半部分的 第 X页/共
          int pageS = writer.getPageNumber();
            for(int i=0;i<pageS;i++){
                ...
                //自己增加的
                Phrase footerLeft = new Phrase("打印时间:"+DateUtils.formatDateTime(new Date()), fontDetail);
                ColumnText.showTextAligned(cb, Element.ALIGN_RIGHT, footerLeft, document.right(), document.bottom() - 25, 0);

                /**
                 * 5.写入页脚1，x轴就是(右margin+左margin + right() -left()- len)/2.0F 再给偏移20F适合人类视觉感受，否则肉眼看上去就太偏左了
                 * y轴就是底边界-20,否则就贴边重叠到数据体里了就不是页脚了；
                 *  注意Y轴是从下往上累加的，最上方的Top值是大于Bottom好几百开外的。
                 */
